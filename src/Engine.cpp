...
Engine::Engine(int width, int height, const char *title)
  ...
  if (!IsWindowReady()) {
    std::cerr << "FATAL: Failed to initialize Raylib window (GLFW context "
                 "creation failed)."
              << std::endl;
  ...
  if (font.texture.id == 0) {
    std::cerr << "WARNING: Failed to load custom font from data/font.ttf or "
                 "../data/font.ttf. Using default font."
              << std::endl;
    font = GetFontDefault();
  } else {
...
void Engine::run() {
  while (!WindowShouldClose()) {
    ...
    if (IsKeyPressed(KEY_R)) {
      if (map) {
        camera.target = {map->getWorldWidth() / 2.0f,
                         map->getWorldHeight() / 2.0f};
        camera.offset = {screenWidth / 2.0f, screenHeight / 2.0f};
        float scaleX = static_cast<float>(screenWidth) / map->getWorldWidth();
        float scaleY = static_cast<float>(screenHeight) / map->getWorldHeight();
        camera.zoom = std::min(scaleX, scaleY);
    ...
    if (map) {
      ...
      if (selectionType == SelectionType::ROAD && selectedRoad) {
        for (size_t i = 0; i < selectedRoad->points.size() - 1; ++i) {
          DrawLineEx(selectedRoad->points[i], selectedRoad->points[i + 1], 4.0f,
                     highlightColor);
        }
...
void Engine::drawUI() {
  ...
  if (selectionType != SelectionType::NONE) {
    ...
    if (selectionType == SelectionType::VEHICLE && selectedVehicle) {
      ...
      drawText(TextFormat("#%ld", selectedIntersection->id), cursorX + 100,
               cursorY - 2, fontSize, valueColor);
      cursorY += lineSpacing;

      drawText("Visits:", cursorX, cursorY, fontSize, labelColor);
      drawText(TextFormat("%ld", selectedIntersection->stats.vehiclesVisited),
      ...
      drawText(TextFormat("%ld", selectedRoad->stats.vehiclesPassed),
               cursorX + 100, cursorY - 2, fontSize, valueColor);
      cursorY += lineSpacing;

      drawText("Avg Speed:", cursorX, cursorY, fontSize, labelColor);
      drawText(TextFormat("%.1f m/s", selectedRoad->stats.getAverageSpeed()),
               cursorX + 100, cursorY - 2, fontSize, valueColor);
      cursorY += lineSpacing;

      drawText("Type:", cursorX, cursorY, fontSize, labelColor);
      drawText(selectedRoad->isOneWay ? "One Way" : "Two Way", cursorX + 100,
               cursorY - 2, fontSize, valueColor);
    }
  }
}
