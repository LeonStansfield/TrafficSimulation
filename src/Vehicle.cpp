...
Vehicle::Vehicle(Vector2 pos, Vector2 sz, Color col, Map *m, Pathfinder *pf)
  ...
  std::uniform_real_distribution<> turning_dist(3.0f,
                                                5.0f); // Radians per second
...
void Vehicle::updateStats(float deltaTime) {
  ...
  if (currentSpeed > 0.1f) {
    ...
    if (currentRoad) {
      currentRoad->stats.addSpeed(currentSpeed);
...
void Vehicle::update(Quadtree *quadtree,
  ...
  if (currentRoadPoints.empty() ||
      currentRoadPointIndex >= currentRoadPoints.size() - 1) {
    ...
    if (currentPath.empty() || currentPathRoadIndex >= currentPath.size() - 1) {
      ...
      if (destinationIntersectionId != -1) {
        ...
        if (intersections.find(destinationIntersectionId) !=
            intersections.end()) {
  ...
  for (size_t i = currentRoadPointIndex;
       i < currentRoadPoints.size() - 2 && distanceTraveled < lookAheadDistance;
       ++i) {
  ...
  float curvature =
      (distanceTraveled > 0) ? totalAngleChange / distanceTraveled : 0;
  ...
  float vehicleLookAhead =
      std::max(minGap * 5.0f, currentSpeed * (desiredTimeGap + 1.0f));
  ...
  if (leadVehicle) {
    ...
    if (distance < safeDistance) {
      float brakeSpeed =
          leadVehicle->getSpeed() + (distance - safeDistance) / desiredTimeGap;
  ...
  if (isWaitingAtJunction) {
    ...
    state = VehicleState::WAITING_JUNCTION;
             ...
             currentRoadPointIndex >= currentRoadPoints.size() - 2) {
    // We are approaching the junction
    float distanceToJunction =
        Vector2Distance(position, currentRoadPoints.back());
  ...
  float currentTurningSpeed =
      maxTurningSpeed - speedT * (maxTurningSpeed - minTurningSpeed);
  ...
  float angle = atan2(desiredDirection.y, desiredDirection.x) -
...
void Vehicle::draw(bool debug) {
  ...
  DrawLineEx(position, Vector2Add(position, Vector2Scale(direction, 15.0f)),
             2.0f, BLACK);
}
...
void Vehicle::requestNewPath() {
  ...
  if (startIntersectionId == 0)
    startIntersectionId = currentRoad->fromIntersectionId;

  // Get new random destination
  destinationIntersectionId = -1;
  while (destinationIntersectionId == -1 ||
         destinationIntersectionId == startIntersectionId) {
    destinationIntersectionId = map->getRandomIntersectionId(gen);
  }

  currentPath =
      pathfinder->findPath(startIntersectionId, destinationIntersectionId);
...
Vector2 Vehicle::getPosition() const { return position; }

Vector2 Vehicle::getDirection() const { return direction; }

float Vehicle::getSpeed() const { return currentSpeed; }
...
Vector2 Vehicle::getSize() const { return size; }
...
int Vehicle::getPathIndex() const { return currentPathRoadIndex; }