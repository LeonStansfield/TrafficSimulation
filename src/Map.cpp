...
public:
  ...
  void way(const osmium::Way &way) {
    ...
    if (highway_tag && drivable_tags.count(highway_tag)) {
      ...
      bool oneWay = (oneway_tag && (strcmp(oneway_tag, "yes") == 0 ||
                                    strcmp(oneway_tag, "1") == 0));
      ...
      if (maxspeed_tag) {
        ...
        } else if (strcmp(highway_tag, "trunk") == 0 ||
                   strcmp(highway_tag, "trunk_link") == 0) {
          speedLimit = 26.8f; // ~60 mph
        } else if (strcmp(highway_tag, "primary") == 0 ||
                   strcmp(highway_tag, "primary_link") == 0) {
          speedLimit = 22.35f; // ~50 mph
        } else if (strcmp(highway_tag, "secondary") == 0 ||
                   strcmp(highway_tag, "secondary_link") == 0) {
          speedLimit = 17.88f; // ~40 mph
        } else if (strcmp(highway_tag, "tertiary") == 0 ||
                   strcmp(highway_tag, "tertiary_link") == 0) {
...
static std::vector<Vector2> offsetPolyline(const std::vector<Vector2> &polyline,
                                           float offset) {
  if (polyline.size() < 2)
    return polyline;
...
static Vector2 GetClosestPointOnLineSegment(Vector2 point, Vector2 p1,
                                            Vector2 p2) {
  ...
  if (segmentLenSqr == 0.0f)
    return p1; // Segment is just a point
...
static void addArrowVerts(std::vector<Vector2> &verts, Vector2 midPoint,
                          Vector2 direction, float size) {
  Vector2 p1 =
      Vector2Add(midPoint, Vector2Scale(direction, size / 2.0f)); // Arrow tip
...
Map::Map(const char *filename) {
  ...
  worldWidth = std::abs(maxLon - minLon) * 111320.0f *
               std::cos(centerLat * M_PI / 180.0f);
  ...
  for (const auto &pair : node_counts) {
    if (pair.second > 1) {
      intersections[pair.first] = {pair.first,
                                   convertLatLonToWorld(nodes[pair.first])};
    }
  }
  ...
  for (const auto &way : ways_temp) {
    if (intersections.find(way.node_ids.front()) == intersections.end()) {
      intersections[way.node_ids.front()] = {
          way.node_ids.front(),
          convertLatLonToWorld(nodes[way.node_ids.front()])};
    }
    if (intersections.find(way.node_ids.back()) == intersections.end()) {
      intersections[way.node_ids.back()] = {
          way.node_ids.back(),
          convertLatLonToWorld(nodes[way.node_ids.back()])};
    }
  }
  ...
  for (const auto &way : ways_temp) {
    ...
    for (size_t i = 1; i < way.node_ids.size(); ++i) { // Start from 1
      ...
      if (intersections.count(way.node_ids[i]) ||
          i == way.node_ids.size() - 1) {

        long startNodeId = way.node_ids[last_intersection_index];
        long endNodeId = way.node_ids[i];

        if (intersections.find(startNodeId) == intersections.end() ||
            intersections.find(endNodeId) == intersections.end()) {
        ...
        std::pair<long, long> segmentId = {std::min(startNodeId, endNodeId),
        ...
        if (!way.isOneWay) {
          ...
          std::reverse(reversed_base_points.begin(),
                       reversed_base_points.end());
          backward_road.points =
              offsetPolyline(reversed_base_points, roadOffset);
  ...
  std::cout << "Number of intersections created: " << intersections.size()
            << std::endl;
  std::cout << "Number of roads created: " << roads.size() << std::endl;
  std::cout << "World dimensions (meters): " << worldWidth << " x "
            << worldHeight << std::endl;
}

Vector2 Map::convertLatLonToWorld(Vector2 latLon) {
  float x = (latLon.y - minLon) * 111320.0f *
            std::cos((minLat + (maxLat - minLat) / 2.0f) * M_PI / 180.0f);
  float y = (maxLat - latLon.x) * 110574.0f;
  return {x, y};
}

void Map::draw(DrawMode mode) {
  DrawRectangleLines(0, 0, static_cast<int>(worldWidth),
                     static_cast<int>(worldHeight), RED);
  ...
  } else if (mode == DrawMode::HEATMAP) {
    // HEATMAP VIEW
    rlDrawRenderBatchActive(); // Flush previous batch
    rlBegin(RL_LINES);

    for (const auto &road : roads) {
      float avgSpeed = road.stats.getAverageSpeed();
      // Avoid division by zero
      // If no samples, assume free flow (congestion = 1.0)
      float congestion = 1.0f;
      if (road.stats.speedSamples > 0 && road.speedLimit > 0.0f) {
        congestion = avgSpeed / road.speedLimit;
      }

      // Congestion ratio: 1.0 = full speed, 0.0 = stopped
      // We want Red = Congested (low ratio), Green = Free (high ratio)
      // Gradient: Green (1.0) -> Orange (0.5) -> Red (0.0)

      auto lerpColor = [](Color c1, Color c2, float t) -> Color {
        return Color{static_cast<unsigned char>(c1.r + t * (c2.r - c1.r)),
                     static_cast<unsigned char>(c1.g + t * (c2.g - c1.g)),
                     static_cast<unsigned char>(c1.b + t * (c2.b - c1.b)), 255};
      };
      ...
      if (points.size() >= 2) {
        ...
        if (isOneWay) {
          ...
          Vector2 leftArrowPos =
              Vector2Add(midPoint, Vector2Scale(normal, 2.5f));
          Vector2 rightArrowPos =
              Vector2Subtract(midPoint, Vector2Scale(normal, 2.5f));
          addArrowVerts(arrowLineVerts, leftArrowPos, dir, 7.0f);
          addArrowVerts(arrowLineVerts, rightArrowPos, Vector2Scale(dir, -1.0f),
                        7.0f);
        }
      }
    }
...
long Map::getRandomIntersectionId(std::mt19937 &rng) const {
  ...
  std::uniform_int_distribution<> dist(
      0, static_cast<int>(intersections.size()) - 1);
