...
bool parseConfigFile(const std::string& path, std::string& mapFile, int& numVehicles, bool& fastMode, int& numTicks) {
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            std::cerr << "Warning: Failed to parse config line: " << line << " (" << e.what() << ")" << std::endl;
        }
    }
    return true;
}
...
std::map<std::string, std::string> parseCmdLine(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key.rfind('-', 0) == 0) { // Check if it starts with '-'
            ...
            std::cerr << "Warning: Flag " << key << " is missing a value or value starts with '-'." << std::endl;
        }
    }
    return args;
}
...
void printUsage(const char* progName) {
    ...
    std::cerr << "  -ConfigFile <path>        Path to configuration file. (Loads first)" << std::endl;
    std::cerr << "  -OSMFile <path>           Path to .osm file. (Required)" << std::endl;
    std::cerr << "  -NumberOfVehicles <int>   Number of vehicles to spawn." << std::endl;
    std::cerr << "  -FastSimulation <bool>    Enable fast-forward mode." << std::endl;
    std::cerr << "  -SimulationTicks <int>    Number of ticks for fast-forward." << std::endl;
    std::cerr << std::endl;
    std::cerr << "Example (Config): " << progName << " -ConfigFile config.ini" << std::endl;
    std::cerr << "Example (Manual): " << progName << " -OSMFile data/MyMap.osm -NumberOfVehicles 500 -FastSimulation true -SimulationTicks 50000" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    if (args.count("-ConfigFile")) {
        ...
        if (!parseConfigFile(args["-ConfigFile"], mapFile, numVehicles, fastMode, numTicks)) {
    ...
    // 4. Validate
    if (mapFile.empty()) {
        std::cerr << "Error: No OSMFile specified in config or arguments." << std::endl;
        printUsage(argv[0]);
        return 1;
    }
