...
bool Quadtree::insert(Vehicle* vehicle) {
    ...
    if (northeast->insert(vehicle)) return true;
    if (northwest->insert(vehicle)) return true;
    if (southeast->insert(vehicle)) return true;
    if (southwest->insert(vehicle)) return true;

    return false;
}

std::vector<Vehicle*> Quadtree::query(Rectangle range) {
...
void Quadtree::draw(const Camera2D& camera) {
